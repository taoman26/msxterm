...
* 空白を含むパスを指定する場合はダブルクォートで囲ってください。
* ファイル名を省略した場合やファイルを作成できない場合はエラーメッセージを表示します（終了はしません）。
* プログラムバッファが空の場合は何も保存せず、その旨を表示します（既存のファイルを空で上書きしません）。
* プログラムバッファに入るのは、ターミナルから入力した行番号付きの行と `#load` したファイルだけです。
  MSX0 側で直接入力・ロードしたプログラムは含まれないため、その場合は先に `#reload_from` で MSX0 側から取り込んでください。
* MSX0 側の内容とズレている場合は `#reload_from` で MSX0 側と同期をとります。
...
* MSX0 に `list` を送り、その出力（行番号付きの行）を取り込みます。`Ok` プロンプトが返ってきた時点で完了です。
* 取り込み中は list の出力は画面に表示されません。完了すると取り込んだ行数が表示されます。
* MSX0 側で BASIC プログラムが実行中の場合は、先に Ctrl-C で停止してから実行してください。
* MSX0 から応答がない場合はタイムアウト（5秒）し、プログラムバッファはそのまま残ります。
* 取り込んだ内容は `#list` で確認でき、`#save` でローカルに保存できます。
